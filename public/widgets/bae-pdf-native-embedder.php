...
class bae_pdf_native_embedder extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'PDF Native embedder', 'b-addon' );
	}
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
            'section_content',
            [
                'label' => esc_html__( 'PDF Content', 'b-addon' )
            ]
        );
		$this->add_control(
			'label_name',
		...
		$this->start_controls_section(
            ...
            [
                'label' => esc_html__( 'PDF Viewer Setting', 'b-addon' ),
            ]
        );
		$this->add_control(
			'height',
		...
		$this->add_control(
            'file_name_text', [
                ...
                'default'        => esc_html__( 'PDF Test File', 'b-addon' ),
                'condition' => [
                    'file_name' => 'yes'
                ]
            ]
		...
		$this->add_control(
            'download_btn_text', [
                ...
                'default'        => esc_html__( 'Download PDF', 'b-addon' ),
                'condition' => [
                    'download_button' => 'yes'
                ]
            ]
		...
		$this->start_controls_section(
			'section_style',
			[
				'label' => esc_html__( 'Layout', 'b-addon' ),
		...
		$this->add_control(
			...
			[
				...
				'size_units' 	=> [ 'px', '%' ],
				'selectors' 	=> [
					'{{WRAPPER}} .my_native iframe' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		$this->add_group_control(
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .native_style h3' => 'color: {{VALUE}};',
				],
			]
		);
		$this->add_group_control(
	...
	protected function render() {
		...
		<?php if($final_pdf_link == ''): ?>
			<center><h3><?php esc_html_e('Paste the pdf file link from setting widget.','b-addon') ?></h3></center>
		<?php else: ?>
		<div class="my_native">

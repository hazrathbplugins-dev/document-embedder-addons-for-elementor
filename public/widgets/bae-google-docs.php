...
class bae_google_docs extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Google Docs', 'b-addon' );
	}

	public function get_icon() {
		return 'bl_icon fas eicon-post-excerpt';
	}
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
            'section_content',
            [
                'label' => esc_html__( 'Google Docs Content', 'b-addon' )
            ]
        );
		...
		$this->start_controls_section(
            ...
            [
                'label' => esc_html__( 'Google Docs Setting', 'b-addon' ),
            ]
        );
		...
		$this->add_control(
            'file_name_text', [
                ...
                'default'        => esc_html__( 'Docs Test File', 'b-addon' ),
                'condition' => [
                    'file_name' => 'yes'
                ]
            ]
	...
	protected function render() {
		...
		<?php if($final_word_link == ''): ?>
			<center>
				<h3>
				<?php echo esc_html__('Paste the word file link from setting widget.','b-addon'); ?></h3>
			</center>
		<?php else: ?>
			<div class="google_docs_style">
				<iframe src="https://view.officeapps.live.com/op/embed.aspx?src=<?php echo esc_url($final_word_link); ?>?download=false"></iframe>
			</div>
		<?php endif; 
	}
}